...
july14th2020calendar config =
    fromTime config utc july14th2020
...
suite =
    describe "initializes calendar"
        ...
        , test "Print functionality" <|
            \_ ->
                july14th2020calendar (Just { startWeekday = Wed })
                    |> print
                    |> (\_ -> Expect.pass)
        , test "Can initialize from time" <|
            \_ ->
                let
                    config =
                        Just { startWeekday = Tue }
                in
                Date.fromCalendarDate 2020 Jul 14
                    |> fromDate config
                    |> print
                    |> Expect.equal (print (july14th2020calendar config))
